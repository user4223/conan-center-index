...
class LibcurlConan(ConanFile):
    ...
    def _patch_cmake(self):
        ...
        if Version(self.version) < "8.10.0":
            replace_in_file(self, cmakelists, "_LIBRARY}", "_LIBRARIES}")
            replace_in_file(self, cmakelists, "_INCLUDE_DIR}", "_INCLUDE_DIRS}")
    ...
    def _generate_with_cmake(self):
        ...
        # check_symbol_exists() is called using zstd_LIBRARIES generated by CMakeDeps
        ...
        if Version(self.version) < "8.10.0":
            cmake_pkg_names["c-ares"] = "CARES"
            cmake_pkg_names["libpsl"] = "LibPSL"
            cmake_pkg_names["libssh2"] = "LibSSH2"

        required_pkgs = {
            "openssl": self.options.with_ssl == "openssl",
            "wolfssl": self.options.with_ssl == "wolfssl",
            "mbedtls": self.options.with_ssl == "mbedtls",
            "libnghttp2": self.options.with_nghttp2,
            "libssh2": self.options.with_libssh2,
            "with_zlib": self.options.with_zlib,
            "brotli": self.options.with_brotli,
            "zstd": self.options.with_zstd,
            "c-ares": self.options.with_c_ares,
            "libpsl": self.options.get_safe("with_libpsl", False),
        }

        deps = CMakeDeps(self)
        for conan_name, required in required_pkgs.items():
            cmake_name = cmake_pkg_names.get(conan_name, conan_name)
            if required:
                if conan_name in cmake_pkg_names:
                    deps.set_property(conan_name, "cmake_file_name", cmake_name)
                    deps.set_property(conan_name, "cmake_additional_variables_prefixes", [cmake_name.upper()])
                tc.variables[f"CMAKE_REQUIRE_FIND_PACKAGE_{cmake_name}"] = True
            else:
                tc.variables[f"CMAKE_DISABLE_FIND_PACKAGE_{cmake_name}"] = True
